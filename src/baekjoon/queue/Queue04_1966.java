...
public class Queue04_1966 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(testCase-- > 0) {
            ...
            while (!queue.isEmpty()) {
                int[] front = queue.poll(); //맨 처음 값
                boolean isFrontMax = true; //맨 처음 값이 가장 중요도가 큰지?

                for (int i = 0; i < queue.size(); i++) {
                    if (front[1] < queue.get(i)[1]) { //첫번째 값보다 다음값의 중요도가 클때
                        queue.offer(front);
                        for (int j = 0; j < i; j++) {
                ...
                //맨 처음 값이 가장 중요도가 컸으면 해당 값은 출력해야 하는 문서
                count++;
                if (front[0] == M) {
                    break;
                }
